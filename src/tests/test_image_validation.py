...
def _attach_broken_images(broken_images, limit=10):
    """깨진 이미지 목록을 Allure 리포트에 첨부"""
    if not broken_images:
        return
    details = "\n".join(
        f"Index: {img.get('index')}, ID: {img.get('resource_id')}, Reason: {img.get('reason')}"
        for img in broken_images[:limit]
    )
    allure.attach(details, name="깨진 이미지 상세", attachment_type=allure.attachment_type.TEXT)
...
def test_check_broken_images_on_home(home_page):
    ...
    with allure.step("검증 결과 확인"):
        _attach_broken_images(broken_images)
...
def test_check_broken_images_tabs_quick(home_page, tab_name, click_method, scroll_count):
    ...
    with allure.step("결과 확인"):
        _attach_broken_images(broken_images, limit=5)
